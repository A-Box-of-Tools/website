...
The longer version of all three, and why a fourth one is worth the trouble, is in [is it safe to upload files to online converters?](https://abox.tools/de/ratgeber/ist-das-hochladen-von-dateien-sicher/)
...
The exceptions, stated rather than buried: this site carries Google advertising and a visit counter, both of which talk to Google and neither of which is handed anything about your files; the [Images to Video](https://abox.tools/de/bilder-in-video-umwandeln/) tool can fetch an image from an address you paste in, which means that server sees your IP; and [Share Text](https://abox.tools/de/text-teilen/) opens one connection purely to introduce two browsers to each other, which stores nothing and never carries your content. The [privacy page](https://abox.tools/de/datenschutz/) sets out all three in full.

If you arrived here from a search for one of the twenty, the equivalents here are an [image compressor](https://abox.tools/de/bild-komprimieren/) that hits a size you name, [images to PDF](https://abox.tools/de/bilder-in-pdf-umwandeln/), [merge PDF](https://abox.tools/de/pdf-zusammenfuegen/), a [PDF compressor](https://abox.tools/de/pdf-verkleinern/), and an [EXIF viewer and remover](https://abox.tools/de/exif-daten-entfernen/) for the hidden data described further up. All free, no account, and none of them has anywhere to send your files.
...
A link back to this page is appreciated and not required. If you repeat the method and get a different answer — sites change, and this is a snapshot of one afternoon — we would rather hear about it than not. The [contact page](https://abox.tools/de/kontakt/) reaches us, and a correction with a byte count attached will be published here.
