# DICOM-Viewer — DCM-Dateien im Browser öffnen

CT, MRT, Röntgen und Ultraschall, mit Fenster, Header und Messungen.

> DCM-Datei öffnen, kostenlos und ohne Installation: CT-, MRT-, Röntgen- und Ultraschallaufnahmen (.dcm) direkt im Browser. Fenster und Zentrum, eine ganze Serie durchblättern, in Millimetern messen, jeden DICOM-Tag lesen und genau sehen, was in der Datei die Patientin oder den Patienten benennt. Nichts wird hochgeladen.
...
## So öffnen Sie eine DCM-Datei (DICOM)
...
### Wie öffne ich eine .dcm-Datei?

Ziehen Sie sie auf das Feld oben auf dieser Seite. Eine `.dcm`-Datei ist eine DICOM-Datei, also das Format, das jedes Gerät in der Radiologie schreibt, und diese Seite öffnet DCM-Dateien: Sie liest die Datei in Ihrem Browser und zeigt die Aufnahme, ohne Installation und ohne Upload. Das geht unter Windows, auf dem Mac, unter Linux, auf einem Chromebook und auf dem Handy gleich, denn das einzige beteiligte Programm ist der Browser, den Sie ohnehin haben. \
\
Haben Sie keine einzelne Datei bekommen, sondern eine CD oder einen Ordner, dann ziehen Sie den ganzen Ordner herüber. Ein CT oder ein MRT ist eine Datei pro Schicht, oft ganz ohne `.dcm` am Ende des Namens, und erst zusammen geöffnet kommt die Aufnahme wieder in ihre Reihenfolge. Die Datei `DICOMDIR` und der Windows-Viewer, der auf der CD mitkam, können bleiben, wo sie sind. Gebraucht wird keines von beiden.

### Was ist eine .dcm-Datei, und warum passiert beim Doppelklick nichts?

Sie ist ein Bild oder eine Bildfolge aus einem medizinischen Gerät im DICOM-Format: ein Header mit mehreren hundert Feldern (wer die Patientin ist, welches Gerät die Aufnahme gemacht hat, wie die Pixel angeordnet sind) und danach die Pixel selbst. Sie ist kein umbenanntes JPEG. Deshalb lehnen eine Foto-App, ein Bildbearbeitungsprogramm und ein Browser-Tab sie ab, und deshalb ist auf einem Rechner, auf dem nie ein medizinischer Viewer installiert war, kein Programm für sie eingetragen. \
\
Nicht jede Datei mit der Endung `.dcm` ist eine Aufnahme. Dieselben drei Buchstaben verwenden auch ein Programm zum Katalogisieren von Datenträgern und Kalibrierdateien für Motorsteuergeräte in der Autoindustrie. Diese Seite prüft die Datei selbst und nicht ihren Namen, und sie sagt es klar, wenn das, was sie bekommen hat, kein DICOM ist.
