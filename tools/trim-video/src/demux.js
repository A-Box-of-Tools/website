 ...
 *
 * It exists so a trim can be done without touching the picture. Once the frames
 * are laid out as a list - where each one is, how big it is, when it is shown,
 * and whether it can be decoded without the one before it - cutting a section
 * out is choosing a run of that list and writing it back. Nothing is decoded,
 * nothing is encoded, and what comes out is byte for byte what went in.
 *
 * The same list is what the exact path walks when the cut has to land between
 * two keyframes, and what draws the keyframe marks under the timeline on the
 * page. The alternative - playing the file and recording what comes out -
 * accepts more containers but costs real time and re-encodes everything, so it
 * is the fallback rather than the first choice.
 *
 * This began as the reader in /crop-video/ and has one addition: a cropper
 * throws the original sample entry away, because a cropped frame needs a new
 * one, while a trim writes it straight back out. So the whole entry, and the
 * display matrix beside it, are carried out of here untouched.
 *
