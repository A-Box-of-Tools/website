 ...
 *
 * It exists so a clip can be played backwards properly. Reversing is the one
 * job on this site that has to read a file from the end, and a video file is
 * the worst possible shape for that: a frame in the middle of a group cannot
 * be decoded without the frames in front of it, so "the last frame" is not
 * something that can simply be picked up. What makes it tractable is the list
 * this reader hands back - where every frame is, when it is shown, and whether
 * it stands on its own - because from that list the groups can be worked out
 * and walked in the other order. src/reverse.js does that walking.
 *
 * The alternative - playing the file and stepping backwards through it with
 * the browser's own player - accepts more containers but costs a seek per
 * frame, so it is the fallback rather than the first choice. It lives in
 * src/playback.js.
 *
 * This is the reader from /trim-video/, unchanged. It is copied rather than
 * shared because a tool here is self-contained: every path in it is relative,
 * nothing it runs comes from outside its own folder, and the build assembles
 * pages without ever touching src/.
 *
