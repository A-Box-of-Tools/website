/**
 * What a string in a content stream actually says, and how wide it is.
 *
 * `(Dear Mr Smith) Tj` looks like it contains the words. It does not
 * necessarily contain anything: the bytes in that string are indices into a
 * font, and a subsetted font is free to number its glyphs however it likes, so
 * the same sentence can be written `(\003\020\021\005)` and mean the same
 * thing. Two questions have to be answered for every byte before this tool can
 * find a word, and both are answered by the font dictionary rather than by the
 * string:
 *
 *   - which character is this, so that "Smith" can be searched for;
 *   - how far does the pen move afterwards, so that the words either side of
 *     a removal can be left where they were.
 *
 * HOW MANY BYTES IS ONE GLYPH
 *
 * A simple font takes one byte per glyph and a composite font takes what its
 * CMap says, which is two for the Identity encodings that every Unicode-capable
 * generator uses. Getting this wrong halves or doubles the text, which is
 * conspicuous, and it also means removing the wrong bytes - which is not
 * conspicuous at all until somebody opens the finished file.
 *
 * WHEN NOTHING HERE CAN ANSWER
 *
 * A symbolic font with no /ToUnicode map and a built-in encoding that lives
 * inside the font program is genuinely unreadable without a font parser this
 * tool does not have. That is reported rather than guessed at: `text.js` marks
 * those glyphs unreadable, the page says so, and the words on it are not
 * offered as something this tool can find. A redaction tool that quietly
 * mapped an unknown glyph to a plausible letter would be inviting somebody to
 * search for a name, see no matches, and conclude the document is clean.
 */

import { base14Widths, encodingByName, glyphText, STANDARD, WIN_ANSI } from './base14.js';
import { lex } from './content.js';
import { decodeStream } from './shared/pdf-filters.js';
import { Name, PdfStream, PdfString } from './shared/pdf-objects.js';

/** Text-space units are thousandths of the font size, throughout. */
const UNITS = 1000;

/**
 * Read one font dictionary.
 *
 * @param {import('./shared/pdf-reader.js').PdfDocument} doc
 * @param {Map} dict the font
 * @returns {Promise<Font>}
 */
export async function readFont(doc, dict) {
  const subtype = doc.get(dict, 'Subtype');
  const kind = subtype instanceof Name ? subtype.value : '';
  const font = kind === 'Type0'
    ? await composite(doc, dict)
    : await simple(doc, dict, kind);

  font.toUnicode = await readToUnicode(doc, dict);
  font.kind = kind;
  return font;
}

/**
 * A font's answers, in the shape the walker in text.js wants them.
 *
 * @typedef {object} Font
 * @property {(bytes: Uint8Array) => {code: number, size: number}[]} split
 * @property {(code: number) => string} text  '' when it cannot be known
 * @property {(code: number) => number} width in thousandths of the font size
 * @property {boolean} singleByte  whether /Tw applies, which it does not to a
 *   composite font except in the one-byte case the spec carves out
 * @property {number} ascent
 * @property {number} descent
 * @property {number} scale  glyph space to text space; 1 except for Type 3
 */

/* ------------------------------------------------------------ simple fonts */

async function simple(doc, dict, kind) {
  const encoding = simpleEncoding(doc, dict, kind);
  const widths = simpleWidths(doc, dict, encoding);
  const { ascent, descent } = vertical(doc, dict);

  // A Type 3 font carries its own coordinate system and its widths are in it,
  // which is the only place in a PDF where a width is not in thousandths.
  const matrix = kind === 'Type3' ? doc.get(dict, 'FontMatrix') : null;
  const scale = Array.isArray(matrix) && Number.isFinite(matrix[0])
    ? matrix[0] * UNITS : 1;

  return {
    split: splitSingleBytes,
    singleByte: true,
    text: (code) => glyphText(encoding[code] ?? ''),
    width: (code) => widths(code) * scale,
    ascent,
    descent,
    scale,
  };
}

function splitSingleBytes(bytes) {
  const out = new Array(bytes.length);
  for (let at = 0; at < bytes.length; at += 1) out[at] = { code: bytes[at], size: 1 };
  return out;
}

/**
 * Codes to glyph names.
 *
 * The base is Standard unless the font says otherwise, and /Differences then
 * overrides individual codes - which is how a document that uses six accented
 * characters ships an encoding six entries long. A symbolic font is meant to
 * be read with the encoding inside its own font program; there is none of that
 * here, so WinAnsi is used as the guess and /ToUnicode is what actually
 * rescues those files.
 */
function simpleEncoding(doc, dict, kind) {
  const table = (kind === 'TrueType' ? WIN_ANSI : STANDARD).slice();
  const encoding = doc.get(dict, 'Encoding');

  const base = encoding instanceof Name
    ? encodingByName(encoding.value)
    : encodingByName(nameOf(doc.get(encoding, 'BaseEncoding')));
  if (base) base.forEach((glyph, code) => { table[code] = glyph; });

  const differences = doc.get(encoding, 'Differences');
  if (Array.isArray(differences)) {
    let code = 0;
    for (const item of differences.map((value) => doc.resolve(value))) {
      if (typeof item === 'number') code = Math.trunc(item);
      else if (item instanceof Name && code >= 0 && code < 256) {
        table[code] = item.value;
        code += 1;
      }
    }
  }

  return table;
}

/** The /Widths array, then the fallbacks a file is allowed to rely on. */
function simpleWidths(doc, dict, encoding) {
  const first = doc.get(dict, 'FirstChar');
  const listed = doc.get(dict, 'Widths');
  const descriptor = doc.get(dict, 'FontDescriptor');
  const missing = doc.get(descriptor, 'MissingWidth');

  const table = new Map();
  if (Array.isArray(listed) && Number.isFinite(first)) {
    listed.forEach((value, index) => {
      const width = doc.resolve(value);
      if (Number.isFinite(width)) table.set(first + index, width);
    });
  }

  // Only when the file gave no widths at all. A font with a /Widths array that
  // happens not to cover one code means zero there, and the specification says
  // so; reaching for a metric table in that case would be inventing one.
  const builtIn = table.size ? null : base14Widths(nameOf(doc.get(dict, 'BaseFont')));

  return (code) => {
    const known = table.get(code);
    if (known !== undefined) return known;
    if (builtIn) return builtIn.width(encoding[code] ?? '');
    if (Number.isFinite(missing)) return missing;
    return table.size ? 0 : 500;
  };
}

/* --------------------------------------------------------- composite fonts */

/**
 * A Type 0 font: a CMap in front of a CID font.
 *
 * Two lookups rather than one. The CMap turns bytes into a CID, and the
 * descendant font turns a CID into a width. Identity-H, which is what every
 * modern generator writes, makes the first step the identity function on
 * two-byte codes - but the mixed-width CMaps are real, and a CMap that says
 * some codes are one byte long and others two cannot be approximated.
 */
async function composite(doc, dict) {
  const descendants = doc.get(dict, 'DescendantFonts');
  const first = doc.resolve(Array.isArray(descendants) ? descendants[0] : null);
  const child = first instanceof Map ? first : new Map();

  const encoding = doc.get(dict, 'Encoding');
  const cmap = encoding instanceof PdfStream
    ? await readCMap(doc, encoding)
    : identityCMap();

  const widths = cidWidths(doc, child);
  const { ascent, descent } = vertical(doc, child);

  return {
    split: (bytes) => cmap.split(bytes),
    // The carve-out in the specification: /Tw applies to a composite font only
    // where a single byte 32 is a code in its own right, which Identity-H and
    // every other two-byte CMap rule out.
    singleByte: cmap.hasSingleBytes,
    text: () => '',
    width: (code) => widths(cmap.cid(code)),
    ascent,
    descent,
    scale: 1,
  };
}

/** Identity-H and Identity-V, and the predefined CJK CMaps, all of which this
 *  reads as two-byte - which every Unicode one of them is. */
function identityCMap() {
  return { split: splitTwoBytes, cid: (code) => code, hasSingleBytes: false };
}

function splitTwoBytes(bytes) {
  const out = [];
  for (let at = 0; at < bytes.length; at += 2) {
    out.push({ code: (bytes[at] << 8) | (bytes[at + 1] ?? 0), size: 2 });
  }
  return out;
}

/** The /W array: runs written either as `c [w w w]` or as `first last w`. */
function cidWidths(doc, child) {
  const fallback = doc.get(child, 'DW');
  const table = new Map();
  const list = doc.get(child, 'W');

  if (Array.isArray(list)) {
    let at = 0;
    while (at < list.length) {
      const start = doc.resolve(list[at]);
      const next = doc.resolve(list[at + 1]);
      if (Array.isArray(next)) {
        next.forEach((value, index) => {
          const width = doc.resolve(value);
          if (Number.isFinite(width)) table.set(start + index, width);
        });
        at += 2;
      } else if (Number.isFinite(start) && Number.isFinite(next)) {
        const width = doc.resolve(list[at + 2]);
        // A run written the wrong way round, and runs long enough to be a
        // denial of service, both appear in files. Neither is worth expanding.
        const last = Math.min(next, start + 65535);
        if (Number.isFinite(width)) {
          for (let cid = start; cid <= last; cid += 1) table.set(cid, width);
        }
        at += 3;
      } else {
        break;
      }
    }
  }

  return (cid) => table.get(cid) ?? (Number.isFinite(fallback) ? fallback : UNITS);
}

/* ----------------------------------------------------------------- CMaps */

/**
 * An embedded CMap, for both jobs it can have: saying how bytes group into
 * codes, and saying which CID each code is.
 *
 * A CMap is PostScript, but the part of it that matters is written as operands
 * followed by a keyword - `<00> <80> 0 endcidrange` - which is exactly the
 * shape `content.js` already reads. So the lexer is the same one the pages use,
 * and the entries arrive as the arguments of the `end...` operator that closes
 * each block.
 */
async function readCMap(doc, stream) {
  const ranges = [];
  const cids = new Map();
  const spans = [];

  let bytes;
  try {
    ({ bytes } = await decodeStream(stream, (value) => doc.resolve(value)));
  } catch {
    return identityCMap();
  }

  for (const op of lex(bytes)) {
    if (op.name === 'endcodespacerange') {
      for (let at = 0; at + 1 < op.args.length; at += 2) {
        const low = op.args[at];
        if (low instanceof PdfString && low.bytes.length) spans.push(low.bytes.length);
      }
    } else if (op.name === 'endcidrange') {
      for (let at = 0; at + 2 < op.args.length; at += 3) {
        const [low, high, first] = op.args.slice(at, at + 3);
        if (low instanceof PdfString && high instanceof PdfString
            && Number.isFinite(first)) {
          ranges.push({ low: codeOf(low.bytes), high: codeOf(high.bytes), first });
        }
      }
    } else if (op.name === 'endcidchar') {
      for (let at = 0; at + 1 < op.args.length; at += 2) {
        const [code, cid] = op.args.slice(at, at + 2);
        if (code instanceof PdfString && Number.isFinite(cid)) {
          cids.set(codeOf(code.bytes), cid);
        }
      }
    }
  }

  const sizes = new Set(spans);
  const oneByte = sizes.has(1);
  const width = sizes.size === 1 ? [...sizes][0] : 0;

  return {
    hasSingleBytes: oneByte,
    // A CMap whose codespace is one width throughout can be split on that
    // width. A mixed one - some CJK encodings are - needs the ranges walked
    // per byte, which is more than this tool has ever met; two bytes is the
    // answer that is right for everything that reaches here in practice.
    split: width === 1 ? splitSingleBytes : splitTwoBytes,
    cid(code) {
      const exact = cids.get(code);
      if (exact !== undefined) return exact;
      for (const range of ranges) {
        if (code >= range.low && code <= range.high) return range.first + (code - range.low);
      }
      return code;
    },
  };
}

/**
 * /ToUnicode: the map that exists for copy and paste, and so for this.
 *
 * It is optional, which is the whole difficulty. When it is there the text is
 * exactly what the document's author intended a reader to copy - including the
 * "ﬁ" that is one glyph and two characters - and nothing else in the file
 * comes close to that. When it is not, the encoding tables are the fallback and
 * a symbolic font is out of reach.
 */
async function readToUnicode(doc, dict) {
  const stream = doc.get(dict, 'ToUnicode');
  if (!(stream instanceof PdfStream)) return null;

  let bytes;
  try {
    ({ bytes } = await decodeStream(stream, (value) => doc.resolve(value)));
  } catch {
    return null;
  }

  const map = new Map();

  for (const op of lex(bytes)) {
    if (op.name === 'endbfchar') {
      for (let at = 0; at + 1 < op.args.length; at += 2) {
        const [code, value] = op.args.slice(at, at + 2);
        if (code instanceof PdfString) map.set(codeOf(code.bytes), unicodeOf(value));
      }
    } else if (op.name === 'endbfrange') {
      for (let at = 0; at + 2 < op.args.length; at += 3) {
        const [low, high, value] = op.args.slice(at, at + 3);
        if (!(low instanceof PdfString) || !(high instanceof PdfString)) continue;
        const from = codeOf(low.bytes);
        const to = Math.min(codeOf(high.bytes), from + 65535);

        if (Array.isArray(value)) {
          for (let code = from; code <= to; code += 1) {
            map.set(code, unicodeOf(value[code - from]));
          }
          continue;
        }
        if (!(value instanceof PdfString)) continue;
        // The destination of a range is incremented in its last byte, which is
        // what lets one entry cover a whole alphabet.
        const text = unicodeOf(value);
        for (let code = from; code <= to; code += 1) {
          map.set(code, step(text, code - from));
        }
      }
    }
  }

  return map.size ? map : null;
}

/** A big-endian code, which is how every CMap writes one. */
function codeOf(bytes) {
  let value = 0;
  for (const byte of bytes.subarray(0, 4)) value = (value << 8) | byte;
  return value >>> 0;
}

/** A destination string, which is UTF-16BE and may be several characters. */
function unicodeOf(value) {
  if (value instanceof Name) return glyphText(value.value);
  if (!(value instanceof PdfString)) return '';
  const { bytes } = value;
  let text = '';
  for (let at = 0; at + 1 < bytes.length; at += 2) {
    text += String.fromCharCode((bytes[at] << 8) | bytes[at + 1]);
  }
  // An odd length is malformed; the byte that is left is a character on its own
  // in every reader that accepts it at all.
  if (bytes.length % 2) text += String.fromCharCode(bytes[bytes.length - 1]);
  return text;
}

/** The nth destination in a bfrange, counting up in the last character. */
function step(text, by) {
  if (!text || !by) return text;
  const last = text.charCodeAt(text.length - 1) + by;
  if (last > 0xffff) return text;
  return text.slice(0, -1) + String.fromCharCode(last);
}

/* ----------------------------------------------------------------- shared */

function vertical(doc, dict) {
  const descriptor = doc.get(dict, 'FontDescriptor');
  const ascent = doc.get(descriptor, 'Ascent');
  const descent = doc.get(descriptor, 'Descent');
  const box = doc.get(descriptor, 'FontBBox');

  const fromBox = Array.isArray(box)
    ? { top: doc.resolve(box[3]), bottom: doc.resolve(box[1]) }
    : {};

  // The defaults are a typical text face, and they only ever decide how tall a
  // black box is drawn - never what is removed.
  return {
    ascent: pick(ascent, fromBox.top, 750),
    descent: pick(descent, fromBox.bottom, -220),
  };
}

function pick(...values) {
  for (const value of values) if (Number.isFinite(value) && value !== 0) return value;
  return 0;
}

function nameOf(value) {
  return value instanceof Name ? value.value : '';
}

/**
 * The characters a run of string bytes shows, with the width of each.
 *
 * @param {Font} font
 * @param {Uint8Array} bytes  one string operand
 * @returns {{code: number, size: number, at: number, text: string,
 *            width: number, known: boolean}[]}
 */
export function glyphsOf(font, bytes) {
  const out = [];
  let at = 0;

  for (const { code, size } of font.split(bytes)) {
    const mapped = font.toUnicode?.get(code);
    const named = font.text(code);
    // /ToUnicode first: it is what the document says its own text is, and where
    // the two disagree - a subset font with a stale encoding - it is right.
    const text = mapped !== undefined && mapped !== '' ? mapped : named;
    out.push({
      code,
      size,
      at,
      text,
      width: font.width(code),
      known: text !== '',
    });
    at += size;
  }

  return out;
}

/** Every font in a resource dictionary, read once and kept by name. */
export async function readFonts(doc, resources) {
  const fonts = new Map();
  const dict = doc.get(resources, 'Font');
  if (!(dict instanceof Map)) return fonts;

  for (const [key, value] of dict) {
    const font = doc.resolve(value);
    if (font instanceof Map) {
      try {
        fonts.set(key, await readFont(doc, font));
      } catch {
        // A font that will not read costs the text drawn with it, which the
        // page reports as unreadable rather than as absent.
      }
    }
  }

  return fonts;
}
