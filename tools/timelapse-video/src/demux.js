 ...
 *
 * It exists so a time-lapse can skip. The sample table it builds says where
 * every frame is and which of them are keyframes, and that is what lets this
 * tool read one frame in a hundred instead of all of them: an hour of footage
 * sped up sixty times needs about a thousand frames out of a hundred thousand,
 * and with this table it decodes a short run around each one rather than the
 * whole file. The alternative - seeking the browser's own player to each
 * instant - accepts more containers and is the fallback rather than the first
 * choice, because a seek decodes from the keyframe in front of it whether or
 ...
 *     second at most, which is less than one sampling interval of any time-lapse
 *     worth making, and honouring one properly means honouring all of them.
 *
