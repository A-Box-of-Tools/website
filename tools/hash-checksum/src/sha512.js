 ...
 *
 * The same shape as SHA-256 with everything twice as wide: 64-bit words,
 * 128-byte blocks, eighty rounds, and eight different rotation amounts.
 * SHA-384 is SHA-512 started from different initial values and cut short at 48
 * bytes, so it is the same code and not a second copy of it.
 *
 * WHY THIS IS WRITTEN IN HALVES
 *
 * JavaScript has no 64-bit integer that is also fast. A number is a double, so
 * its bitwise operators work on 32 bits and nothing wider; BigInt is exact and
 * arbitrarily wide and allocates on every operation, which in a loop that runs
 * eighty times per 128 bytes is the difference between a few seconds and a few
 * minutes on a disk image. So each 64-bit word is carried as two 32-bit halves,
 * hi and lo, and every rotation below is written out in terms of them.
 *
 * The rule for reading them: rotating right by n under 32 mixes each half with
 * the other; rotating by 32 or more is the same thing with the halves swapped
 * first, which is why a rotation by 41 appears here as a shift by 9 with hi and
 * lo the other way round.
 *
 * Additions carry by hand. The low halves are added as ordinary numbers - five
 * of them at most, so under 2^35, which a double still holds exactly - and the
...
/** Square roots of the first eight primes: the initial state for SHA-512. */
...
/** Square roots of the ninth to sixteenth primes: the initial state for SHA-384. */
...
/**
 * @param {Int32Array} initHi   the initial state, high halves
 * @param {Int32Array} initLo   and the low halves
 * @param {number} outBytes     64 for SHA-512, 48 for SHA-384
...
function sha512Family(initHi, initLo, outBytes) {
  ...
  const compress = (view) => {
    ...
    for (let i = 16; i < 80; i += 1) {
      const xh = wHi[i - 15];
      const xl = wLo[i - 15];
      // rotate right 1, rotate right 8, shift right 7
      const s0h = ((xh >>> 1) | (xl << 31)) ^ ((xh >>> 8) | (xl << 24)) ^ (xh >>> 7);
      const s0l = ((xl >>> 1) | (xh << 31)) ^ ((xl >>> 8) | (xh << 24)) ^ ((xl >>> 7) | (xh << 25));

      const yh = wHi[i - 2];
      const yl = wLo[i - 2];
      // rotate right 19, rotate right 61 - which is a rotate left by 3 - and shift right 6
    ...
    for (let i = 0; i < 80; i += 1) {
      // Sigma1(e): rotate right 14, 18 and 41
      ...
      // Sigma0(a): rotate right 28, 34 and 39
  ...
  return {
    ...
    digest() {
      // The length field is 128 bits wide here. Its top 64 bits are zero for
      // every file that will ever exist - they would first matter at two
      // exabytes - and the padding has already zeroed them.
...
/** A running SHA-384: SHA-512 from a different start, cut to 48 bytes. */
