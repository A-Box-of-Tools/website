 ...
 * mirrors and FTP sites published it for twenty years and nobody went back. A
 * tool that would not compute one would be refusing to answer the question its
 * visitors actually arrived with. What it can do instead is say plainly what
 * the answer is worth, which is what the page around this file does.
 *
 * The browser will not help here: crypto.subtle knows SHA and nothing else, on
 * the reasonable grounds that a new application should not be reaching for
 ...
 *
 * Written out rather than computed. Math.sin is not required to give the same
 * answer in every engine - the specification lets an implementation choose its
 * own approximation - so deriving these at load time would make the digest
 * depend on the browser, which is the one thing a checksum may never do.
 */
...
/** The per-round rotation, four values reused across each round of sixteen. */
...
export function md5() {
  ...
  const compress = (view) => {
    // MD5 is the only one of these four that reads its words little-endian,
    // which is the single most common reason a hand-written copy of it returns
    // a digest that is right in every bit and in the wrong order.
