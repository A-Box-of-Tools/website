/**
 * The tables from ISO/IEC 18004 that a QR reader cannot derive for itself, and
 * the arithmetic for everything it can.
 *
 * Two things live here rather than in the decoder. The first is the pair of
 ...
 *
 * THIS IS THE SAME FILE AS tools/qr-barcode/src/qr-tables.js, on purpose. A
 * reader that disagreed with the writer next door about the size of a symbol,
 * or about how its blocks are split, would not be usefully different from it -
 * it would be wrong. The two copies exist because a tool ships only its own
 * src/ (see "Shared parts" in README.md), and tests/python/test_duplicates.py
