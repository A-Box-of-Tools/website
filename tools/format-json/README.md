...
*Lay it out, minify it, or convert it — on your own machine.*  ·  lives at `/format-json/`  ·  [all tools](../)  ·  [how the site is built](../../README.md)

This page, [`compare-text`](../compare-text/) and [`encode-text`](../encode-text/)
used to be one tool at `/text-tools/`, four jobs behind four tabs. The split
gave each job the address people actually search for; what stayed here is the
half the tabs were really about — laying out JSON, XML, HTML, CSS and YAML,
and converting JSON to YAML or XML and back. The old address redirects; see
`[redirects]` in `config/site.toml`.
...
| Comparing two texts | [`compare-text`](../compare-text/), since the split |
| Base64 and the other encodings | [`encode-text`](../encode-text/), since the split |
