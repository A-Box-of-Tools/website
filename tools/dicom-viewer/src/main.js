 ...
 *
 * A punctuation mark rather than a word, so that it needs no translation and
 * cannot be mistaken for a value the file actually holds.
 */
const NOTHING = '—';

/** Tag rows drawn before the "show the rest" button. A header can run to thousands. */
const FIRST_PAGE = 250;

/**
 * How many decoded frames to keep.
 *
 * Enough that scrolling back one slice is instant and small enough that a study
 * cannot fill memory: eight frames of a 512x512 CT held as 32-bit values is
 * 8 MB. Everything else is read off the disk again, which is what the File
 * object is for - the browser never stopped being able to reach the file.
 */
const CACHE_FRAMES = 8;

/** @type {{series: object[], index: number, positions: object[]}|null} */
let stack = null;
/** @type {object|null} the file whose header is on screen */
let open = null;
/** @type {object|null} the frame being drawn */
let shown = null;
...
const cache = new Map();
/**
 * Which series the window on screen was chosen for.
 *
 * The window belongs to the series and not to the file, and in a series every
 * file is a different file - so "has the file changed" is the wrong question to
 * hang it on. See the note in `draw`.
 */
...
/**
 * What one file contributes to the arrangement, from as little of it as
 * possible.
 *
 * The header of a DICOM file sits in front of the pixels, and the pixels are
 * all of the size. Reading the first slice of the file is enough to place it in
 * its series in the overwhelming majority of cases, and on a folder of three
 * hundred CT slices that is the difference between reading four megabytes and
 * reading a hundred and fifty.
 *
 * Where the head is not enough - a deflated file, or a header longer than the
 * slice, or one whose image size is somewhere past it - the whole file is read.
 * Correctness first; the short read is only ever an optimisation that can
 * decline.
 */
...
/**
 * One file, all the way through: the wrapper, the dataset, and what it says
 * about its own pixels.
 *
 * Asynchronous for one transfer syntax only. Deflated Explicit VR Little Endian
 * holds the dataset as a raw DEFLATE stream, which the browser will inflate and
 * which it will only do asynchronously - so the parser stays synchronous and
 * this is where the one await lives.
 */
...
/** DEFLATE, through the browser's own inflater. */
...
/**
 * A series, laid out as a single line of positions to scroll through.
 *
 * A stack is not a list of files. A file may hold one frame or two hundred, and
 * an ultrasound loop of two hundred frames and a CT of two hundred slices are
 * the same gesture to whoever is looking at them. So the scrubber addresses
 * (file, frame) pairs, and the label says which of the two it is stepping
 * through.
 */
...
/**
 * One position of the stack, on screen.
 *
 * Two different "has it changed" questions, and conflating them is a bug that
 * is easy to write and unpleasant to use. The header panel and the tag table
 * belong to the *file* and are rebuilt whenever it changes. The window and the
 * zoom belong to the *series*, and in a series every slice is a different file
 * - so hanging them on the file makes every step of the scroll snap the window
 * back, and the anatomy appears to change brightness as it goes. Comparing
 * slices under one window is the entire reason for scrolling a stack.
 */
async function showPosition(at, fresh) {
  ...
  // viewport, and a card that is still `hidden` measures nought by nought - so
  // the canvas would be sized to zero and the picture, though decoded and
  // correct, would be a blank line where the viewer should be.
  ...
  // The frame count came off the header read when the files were opened, so
  // this almost never fires. It is here for the file whose header ran past the
  // quarter-megabyte that read, where the count is only known now.
...
/**
 * Give one file the number of positions it turns out to need, and say where in
 * the stack it now begins.
 *
 * The whole contiguous run belonging to that file is replaced rather than the
 * one position that was wrong, because a file that is now known to hold three
 * frames instead of twenty leaves seventeen positions behind that address
 * nothing.
 */
...
/**
 * Baseline JPEG, through the decoder the browser already has.
 *
 * The one place this tool leans on something it did not write, and it is worth
 * being plain about what that costs: `createImageBitmap` hands back eight-bit
 * RGB, so a twelve-bit JPEG loses four bits and a greyscale one arrives as
 * three identical channels. The page says so rather than presenting the result
 * as though the file's own precision had survived.
 *
 * The alternative was a second JPEG decoder written out by hand beside the
 * lossless one, to produce a worse picture than the browser's more slowly.
 */
 ...
 *
 * A window the *file* named keeps the file's own word for it - `LUNG`,
 * `MEDIASTINUM` - because that is data out of the header rather than something
 * this page has to say. Everything else is this page's own wording and comes
 * off the markup, where a translator can reach it.
 */
...
/**
 * How much taller a pixel is than it is wide.
 *
 * Non-square pixels are normal outside CT: an ultrasound or an old fluoroscopy
 * image is routinely stored with rectangular ones, and drawing them square
 * makes a circle an ellipse. Which matters here beyond appearances, because the
 * measurement tool draws on the same picture.
 */
...
/** Where the canvas sits inside the viewport, at the current zoom and pan. */
function place() {
  if (!shown) return;
  ...
  // Below 1:1 the browser's smoothing is a genuine reduction and helps. Above
  // it, smoothing would invent detail that is not in the scan, which on a
  // medical image is the one thing a viewer must not do.
...
el.viewport.addEventListener('pointermove', (event) => {
  ...
  if (view.mode === 'window') {
    // A drag has to move the window by an amount that depends on how wide the
    // range is: a hundred units across a CT is a visible change and across an
    // ultrasound's 0-255 it is the whole picture. A pixel is worth a two
    // hundredth of the frame's own range in both.
...
el.viewport.addEventListener('wheel', (event) => {
  ...
  // Zoom about the cursor rather than the centre, so that what is under the
  // pointer stays under it. Anything else means chasing the detail across the
  // frame as it grows.
...
/** Where in the frame's own pixels a pointer event landed. */
...
/** What the file stores at the pixel under the cursor, and what that means. */
...
/** A line drawn on the picture, and how long it is where the file allows. */
function measure(from, to) {
  ...
  if (spacing) {
    // The two axes have their own spacing, so the distance is measured in
    // millimetres on each axis and combined - not measured in pixels and
    // multiplied by one of them, which is right only where the pixels are square.
...
function updateScrubLabel(at) {
  ...
  const parts = [];
  // "1 of 6 · frame 1 of 6" on a single multi-frame file says the same thing
  // twice, so the position is only worth naming where there is more than one
  // file to be at a position in.
  if (series.instances.length > 1) {
...
/**
 * The value range, which is a fact about the decoded pixels rather than about
 * the header, and so is filled in by `paint` rather than with the rest of the
 * panel beside it.
 */
...
function matching() {
  ...
  // A tag typed any of the ways people write one - `0010,0010`, `(0010,0010)`,
  // `00100010` - matched against the eight bare hex digits everything is keyed
  // by. Only where what is left really is a tag: stripping the non-hex
  // characters out of `window` leaves `d`, and a search for a word should not
  // quietly become a search for every tag with a `d` in its number.
...
/** The line above the table, which is four different sentences. */
...
el.saveFrame.addEventListener('click', () => {
  if (!shown) return;
  // The stored pixels at their own size, under the window on screen, and
  // nothing else: no overlay, no measurement, no patient details burned in.
  // A picture that carries the name back out of the file would undo the point
