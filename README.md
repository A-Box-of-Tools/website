...
| Audio: trim, fade, speed, volume, waveform | `decodeAudioData` and `OfflineAudioContext`. WAV out is a 44-byte header in front of the samples. Reversing, speed and level are built: `/edit-audio/`, where the two things worth reading are the time-stretcher and the reason the file's own sample rate has to be read before it is decoded |
