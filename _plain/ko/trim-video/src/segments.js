/**
 * Segments: the list of in-and-out points you mark while a video plays.
 *
 ...
 * to do: watch a recording once, mark the six moments worth keeping as they go
 * past, and get those six back as one video with the rest gone.
 *
 * So a video here holds a *list* of segments rather than a selection:
 *
 ...
 *     delete, and the total of their lengths is what the finished video will
 *     run to.
 *
 * ## The timestamps file
 *
 * The marks can be saved as a plain text file and read back, in the same two
 * formats the tool this one is modelled on writes:
 *
 ...
 * one segment. Keeping to that layout is deliberate: a file written here can be
 * fed to that tool's `cutVideoByTimestamps.py`, and a file written by it can be
 * dropped onto this page. Marking is careful work and nobody should have to do
 * it twice because two programs could not agree on a comma.
 *
 * Nothing in this file touches a video. It turns times into text and back.
 */
...
/** How long the finished video will run to. */
 ...
 * @param {string} name  what was marked. Written into the header, where the
 *   tool this format comes from puts a YouTube id.
 */
