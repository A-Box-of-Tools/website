 ...
 *
 * It exists so the crop can be done properly: decode the real frames, crop
 * them, encode them again, and keep the original timing and the original audio
 * exactly as they were. The alternative - playing the file and recording what
 * comes out - accepts more containers but costs real time and re-encodes the
 * sound, so it is the fallback rather than the first choice.
 *
