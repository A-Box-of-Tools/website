/**
 * Putting one frame on the canvas the GIF is built from.
 *
 * Both ways of reading a video go through here, so a clip decoded by WebCodecs
 * and the same clip played by the browser land on exactly the same pixels.
 *
 ...
 * the browser skip compositing; `willReadFrequently` because every frame is
 * read straight back out with getImageData, which is the one access pattern
 * that is slower on a GPU-backed canvas than on a software one.
 */
