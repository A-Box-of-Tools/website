 ...
 * The tag dictionary.
 *
 * EXIF is a TIFF file hiding inside a photo, and a TIFF tag is a bare number:
 * the file says 0x829a, not "ExposureTime". Everything a person reads on this
 * page comes out of the table below.
 *
 * Three things are recorded per tag, and each one is used somewhere:
 *
 *   name   what to call it on screen.
 *   edit   how the value may be changed, or absent for read-only. A tag with
 *          no editor can still be deleted - deleting never needs to understand
 *          the value, only writing a new one does.
 *   risk   'high' if the tag can identify a person, a place or a device;
 *          'medium' if it says where and when you were in a looser sense.
 *          src/report.js turns these into the findings list.
 *
 * The list is deliberately not exhaustive. Unknown tags are still shown, still
 * counted, and still removed - they appear as "Tag 0x9999" instead of a name,
 * which is honest about what we know rather than pretending the file is empty.
 */
 ...
 * Bit 0 is whether it fired; bit 5 says whether the camera even has one.
 */
...
/** IFD0 and IFD1 - the tags that describe the picture as a whole. */
...
/** The GPS sub-IFD. Every tag in it is a location, so the whole group is high risk. */
...
/** The interoperability sub-IFD. Two tags, neither interesting, both removable. */
export const INTEROP_TAGS = {
  0x0001: { name: 'Interoperability index' },
  0x0002: { name: 'Interoperability version' },
};

/** Group id -> dictionary. Group ids are the keys used throughout the app. */
...
/**
 * Look a tag up. Unknown tags get a name that at least says which number they
 * are, because "we do not have a label for this" and "there is nothing here"
 * are very different statements to make about someone's photo.
 */
