 ...
 *
 * It exists so that the frames can be sampled at the instants a GIF actually
 * wants them. Once the file is laid out as a list - where each frame is, how
 * big it is, and when it is shown - answering "what was on screen at 4.25
 * seconds" is a lookup rather than a seek, and the answer is the frame the file
 * says it is rather than whichever one a player chose to land on. The
 * alternative - seeking a <video> element to every instant - accepts more
 * containers and is slower and vaguer, so it is the fallback rather than the
 * first choice.
 *
 * This began as the reader in /crop-video/ and is unchanged apart from this
 * note. Nothing here is GIF-specific; it is the audio half that a GIF has no
 * use for, and it is left in rather than pruned, so that the two copies can
 * still be diffed against each other.
 *
