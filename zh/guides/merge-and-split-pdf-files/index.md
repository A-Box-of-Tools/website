...
把两份文件拼到一起，是人们对 PDF 做得最平常的一件事，也是最常常靠把两个文件交给陌生人的服务器来完成的一件事。其实用不着。下面说的是怎么做，以及一个工具重排页面时，有什么会悄悄不见。

[打开PDF 合并与拆分](https://abox.tools/zh/merge-pdf/): 挪动页面，不用往服务器跑一趟。

最后更新 2026年8月26日

## 简短的回答

打开 [PDF 合并与拆分](https://abox.tools/zh/merge-pdf/)，把要用的每个文件都拖进去，再把页面拖成你要的顺序。然后说清楚它是出成一份文件还是好几份，按下按钮。什么都不会被上传：文件是你自己的浏览器打开、拆散、再写回去的。
...
![输出卡片：一份文档还是多份的选项，按大小拆分、在某一页拆分，或者拆回它原本的那几个文件。](https://abox.tools/screens/merge-and-split-pdf-files/output.webp)

拆分和合并是同一个动作反着跑，所以它是这里的一个设置，而不是另一个工具。
...
卡片上的编号会随着你的操作重编，所以它们永远表示「在成品文件里的位置」，而不是「在原来那个文件里的第几页」。在你按下按钮之前，什么都还没写出去，所以也没有什么需要撤销——而「恢复成刚进来的样子」会把一切的原始顺序还回来。

![页面网格：两份文档的每一页都以缩略图按输出顺序排列，带有旋转、翻转和移除的控件。](https://abox.tools/screens/merge-and-split-pdf-files/pages.webp)

两份文档，一页一页地看。重新排序就是拖动；上面那个范围输入框，是给拖起来要花掉一个下午的文档准备的。
...
- **填好的表单能留下**，而且新文件会被登记成表单，好让阅读器把它当表单对待。有个值得知道的怪癖：同名的两个字段，在任何阅读器看来都是*同一个*字段，所以把同一张表单的两份合到一起，它们就被绑在了一块儿——在一个里打字，另一个也会跟着填上。
...
加密过的 PDF 会被拒收，包括很多办公室复印机产出的那种空密码的。去掉一份文件的保护，和挪它的页面是两件不同的事，一个悄悄替你去做的工具，做的是你没要求过的事。请先用密码在阅读器里打开它，另存一份没有保护的副本。
...
而这件事在这儿比在几乎任何别处都要紧，因为人们合并的是*什么*。会被拼到一起的文件，都是从某处来的：一份合同和它的签字页、一张护照扫描件和一份银行流水、一封医生的信和一张申请表。一个在线合并服务一次就收到了它们全部，已经归拢好，来自同一个人。这是大多数人一辈子做过的最能说明问题的一次上传。

这里的工具没有任何网络功能，页面的 `Content-Security-Policy` 里写着它允许联系的每一个地址，没有一个属于本站。把页面加载好，拔掉网线，照样合一份试试。

[把文件传给在线转换器安全吗？](https://abox.tools/zh/guides/is-it-safe-to-upload-files/)那一页还列了另外三条这样的检查。
