...
[打开PDF 解锁](https://abox.tools/zh/unlock-pdf/): 大多数「加了密」的 PDF 其实根本不用密码。这一页会先告诉你手上是哪一种，再动它。

最后更新 10 September 2026

## The short answer

Open the [PDF Unlocker](https://abox.tools/zh/unlock-pdf/) and drop the document in. Within a second it will tell you which of two situations you are in, and they are not close to each other:
...
In most readers you can see the detail: open the document properties and look at the Security tab, which lists every permission as Allowed or Not Allowed. The [PDF Unlocker](https://abox.tools/zh/unlock-pdf/) shows the same list, and adds the thing no reader shows you — which encryption scheme the file uses, and what that scheme is worth today.
...
An unlocked document is an ordinary one, so the rest of the box works on it: [merge or split it](https://abox.tools/zh/merge-pdf/), [make it smaller](https://abox.tools/zh/compress-pdf/), or [take a name out of it properly](https://abox.tools/zh/redact-pdf/) — which, if the reason it was locked was that it contains something private, is very likely the next thing you actually want.
