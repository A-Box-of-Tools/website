...
把播放器暂停、按下截屏键，你得到的是一张窗口的图。有时候这就够了。下面说的是两者差在哪儿，以及在真的要紧的时候，怎么把那一帧本身拿出来。

[打开视频截帧](https://abox.tools/zh/grab-frame/): 任意位置，一张原画质静帧。

最后更新 2026年8月26日

## 简短的回答

打开[视频截帧](https://abox.tools/zh/grab-frame/)，把片子拖进去，找到那个瞬间，按下*截取这一帧*。落到下载文件夹里的，是按视频自己的分辨率出的那一帧——4K 的片子就是 ⁦3840 × 2160⁩，跟页面上的预览画面有多大没有关系。
...
截帧工具把这四样全跳过：它把文件里真正装着的那一帧解出来，再把那些像素写下去。图有多大，视频就有多大，而且没人在上面画过东西。
...
![找帧界面：一张烧了时间码的静帧、一条拖动条、逐帧按钮，以及给出精确时刻和帧号的输入框。](https://abox.tools/screens/grab-a-frame-from-a-video/find.webp)

一帧一帧地走，才能落在你想要的那一帧上。时刻和帧号指的是同一张画面，两个都可以直接输入。
...
有一点值得说清楚：从视频里取出来的一帧，本来就已经是压过的画面了。存成 PNG 并不能把那一层撤销，也拿不回编码器当初拍摄时扔掉的细节。PNG 给你的，是不让东西被扔*第二遍*。要是这张图之后还要调色或者裁切，这有意义；要是只是发给别人，那没有。
...
![从同一段片子里取的三张静帧，以缩略图和各自的时刻排列，还有一个把它们一次全部保存的按钮。](https://abox.tools/screens/grab-a-frame-from-a-video/shots.webp)

多取几张，回头再挑。保存之前它们都留在页面里，而保存只是一个按钮。
...
要是这张图接下来还得换个尺寸或形状，那就当作单独一步来做：[图片缩放](https://abox.tools/zh/resize-image/)能缩放、裁切和转换，[它的指南](https://abox.tools/zh/guides/resize-an-image/)里写了这几件事各要付什么。
