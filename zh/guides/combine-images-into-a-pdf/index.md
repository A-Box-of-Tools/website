...
人家要“一个 PDF”，而你手上是十一张纸的照片。这一页讲那些真正改变结果的选择：页面大小、顺序、画质，还有这份文档会把你交代多少，以及哪些其实不用管。

[打开图片转 PDF](https://abox.tools/zh/images-to-pdf/): 把你的图片装进一个文档。

最后更新 2026年8月26日

## 先说结论

打开[图片转 PDF](https://abox.tools/zh/images-to-pdf/)，把图片丢进去，拖动卡片把顺序排好，然后生成文档。默认设置是 A4 页面、一点点页边距、照片原样抄进去不重编，多数人要的就是这个。

值得多想一下的有四件：顺序、页面大小、画质设置，还有这份文档会把你交代多少。按出错的频率排，就是这个顺序。

![PDF 第一页的预览，旁边是摘要：四页，页面尺寸随每张图片，四张图片全部原样收录。](https://abox.tools/screens/combine-images-into-a-pdf/preview.webp)

预览才是值得看的那一眼：它就是成品页，形状也和成品页一样。
...
别的格式没法这么搭便车。PNG、WebP、HEIC 这些在 PDF 里没有对应的滤镜，只能转换。怎么转，你可以选：
...
![页面设置：尺寸、方向、图片与页面的贴合方式、页边距和背景色。](https://abox.tools/screens/combine-images-into-a-pdf/layout.webp)

页面尺寸和贴合方式共同决定照片是完整呈现还是按纸张裁掉。选择随每张图片的尺寸，则干脆让这个问题不存在。
...
另外还有图片本身。你的照片要是带着 EXIF 和 GPS 标签，它们的下场取决于走哪条路：一张不重编、直接抄进去的 JPEG，里面有什么就还是什么；而被重新编码的图片，标签会作为副产品消失。你要是在意，那就先用 [EXIF 查看与清除](https://abox.tools/zh/exif-editor/)把照片清一遍，里面到底有什么，[它那篇指南](https://abox.tools/zh/guides/remove-exif-and-gps-data/)讲了。
...
**把做好的文档再压一遍。**[PDF 压缩](https://abox.tools/zh/compress-pdf/)是按每张图在页面上实际画多大来算的，不是按它的像素数，而这才是要紧的那个量度；这一下的代价，[它那篇指南](https://abox.tools/zh/guides/make-a-pdf-smaller/)里讲了。
...
这一条怎么自己核实，不管在这儿还是在别处，[把文件传给在线转换器安全吗？](https://abox.tools/zh/guides/is-it-safe-to-upload-files/)里写了。
