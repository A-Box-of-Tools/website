...
有人扔给你一个数字，100 KB、500 KB、2 MB，而你的照片差得老远。这一页讲的是：这个数字要你拿什么去换，该花在哪儿，以及怎么判断压完的这张还能不能发出去。

[打开图片压缩](https://abox.tools/zh/compress-image/): 说个大小，剩下的它来算。

最后更新 2026年8月26日

## 先说结论

打开[图片压缩](https://abox.tools/zh/compress-image/)，把照片丢进去，把人家给你的那个数字打进去，按按钮。它会把图编码好几遍，留下能塞进目标、又最好的那一份，并告诉你这一下花了多少。老实说，大多数照片配大多数目标，你根本看不出差别。

这一页剩下的部分，是给看得出差别的情况准备的：结果发糊了、PNG 几乎压不动，或者你想在把图发给别人之前，先弄清这个工具到底对它做了什么。

![目标卡片：填入 200 kB，旁边是常见上限的按钮、格式菜单，以及一行说明工具将要尝试什么。](https://abox.tools/screens/compress-an-image-to-a-target-size/target.webp)

把别人给你的数字填进去。下面的一切都是工具朝这个数字使劲，而不是你在质量滑块上瞎猜。

## 一个文件大小限制，实际上是在要什么

JPEG 和 WebP 存的不是你的照片，是对这张照片的一段描述，而质量设置决定这段描述允许有多细。调低它，文件就小，因为描述变糙了：细腻的纹理被平均掉，渐变出现色带，边缘泛起一圈淡淡的块状光晕。

所以，大小限制其实是给细节定的一笔预算。真正该问的不是“我能不能压到 500 KB”，任何数字你都压得到，而是“为了压到这儿，我得放弃画面里的多少东西，而这些东西对我这次的用途要紧不要紧”。
...
有时候那些像素本来就是多余的。一张 4000 像素宽的照片，放在网页上一栏 600 像素宽的位置里，扛着的是六倍于谁能看见的细节。你要是清楚这张图最终待在哪儿，先按那个尺寸缩一下，大小问题往往就没了，一点质量都不用花。干这活的是[图片缩放](https://abox.tools/zh/resize-image/)，怎么挑尺寸，[它自己那篇指南](https://abox.tools/zh/guides/resize-an-image/)里讲了。
...
- **存成了 PNG 的照片。**非常常见，多半是无意的，也是这一页里最容易捡的便宜：转成 JPEG 或者 WebP，往往能小上五到十倍，肉眼还看不出变化。
- **截图或者示意图。**转 WebP。你要它无损它就能无损，而同样的像素，它一般比 PNG 小。转 JPEG 会把文字边缘搞毛。
- **带透明的 logo。**WebP 保得住透明；JPEG 会拿一块纯色把它填掉，而那几乎从来不是你想要的。
...
![一行结果：原图 1.4 MB，压缩后的副本 196 kB，达到这个大小所用的质量，以及一个对比两者的链接。](https://abox.tools/screens/compress-an-image-to-a-target-size/results.webp)

真正出来的东西，就摆在放进去的东西旁边。那个对比链接，是你弄清这个数字有没有让你付出看得见的代价的入口。

## 发文件之前，有三件事值得知道

**压缩会顺带清掉元数据。**重新编码就是把画面解码成像素、再把像素编回去，而一张画布上只有像素，不带任何标签，所以 GPS 位置、相机型号、时间戳这些，干脆就没写进新文件。通常这算白捡的好处。你要是只想清掉标签、画面别动，那是另一件活：[EXIF 查看与清除](https://abox.tools/zh/exif-editor/)只重写容器，不重新压缩任何东西，里面到底有些什么，[它那篇指南](https://abox.tools/zh/guides/remove-exif-and-gps-data/)讲得很清楚。
...
想确认这一点，不管在这儿还是在别处，最省事的办法都是：把页面打开，断开网络，照压一张试试。还能用，就说明什么都没上传。[把文件传给在线转换器安全吗？](https://abox.tools/zh/guides/is-it-safe-to-upload-files/)里还有另外三个同类检查。
