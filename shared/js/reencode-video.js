/**
 * A whole video track, decoded, drawn on to a canvas of the output size, and
 * encoded again as H.264 - the picture half of every tool that writes a new
 * MP4 from an old one.
 *
 * GENERATED INTO EACH TOOL. This file lives at shared/js/reencode-video.js
 * and the build copies it to <tool>/src/shared/reencode-video.js for the
 * tools that ask for it with `js_parts = ["reencode-video", "mp4-reader",
 * "mp4-writer", "mp4-boxes", "frame-canvas", "video-support",
 * "codec-support", "webcodecs", "errors", ...]`. The compressor and the
 * converter both run this loop; what differs between them is only what they
 * ask it for - a smaller frame at a bitrate a number allows, or the same
 * frame at a bitrate the old codec's makes sensible - and what they put
 * beside the result.
 *
 * The shape is the trimmer's exact path with the cuts taken out: every
 * sample in file order into a VideoDecoder, every frame it hands back drawn
 * on to one canvas and handed to a VideoEncoder, every chunk the encoder
 * hands back kept until the end and given a duration worked out from the
 * gap to the next. It reads from whichever reader found the samples - the
 * MP4 one or the Matroska one - because both hand back offsets into the
 * file and a presentation time on a clock, and that is all this needs.
 *
...
import { avcSampleEntry } from './mp4-writer.js';
import { drawScaled, frameCanvas } from './frame-canvas.js';
import { pickH264Codec } from './video-support.js';
import { decoderConfig, micros, settle } from './webcodecs.js';
 ...
 * @param {object} args.video  the track, from demux() or demuxMatroska()
 * @param {{width: number, height: number}} args.frame  the output size
 * @param {number} args.bitrate  bits per second
 * @param {number} args.fps
 * @param {(progress: {phase: string, done: number, total: number}) => void} [args.onProgress]
 * @param {AbortSignal} [args.signal]
 * @returns {Promise<{samples: object[], sampleEntry: Uint8Array, codec: string,
 *   frames: number}>} the samples ready for an Mp4Writer track at
 *   VIDEO_TIMESCALE, and the `avc1` entry that describes them
 */
export async function reencodeVideo({
  ...
  encoder.configure({
    ...
    bitrate,
    framerate,
    // Held to the bitrate rather than allowed to wander around it: a page
    // that promised a size, or said what the picture would spend, means it.
