# How to open a DCM file, and what is in one
...
Open the [DICOM Viewer](https://abox.tools/dicom-viewer/) and drag the `.dcm` file onto it, or the whole folder of them. They are read on your own machine, put back into the series they came from, and stacked in the order the scanner took them in. Nothing is uploaded, and nothing is written back to your files.
...
## What a DCM file is, and what opens one

A file ending in `.dcm` is a DICOM file: the format every medical scanner writes, and the three letters are simply the extension it is usually given. Double-click one on a computer that has never had a medical viewer installed and you are asked what to open it with, because nothing that came with the machine is registered for it. A photo app will not do. The pixels inside are not a JPEG under another name; they are twelve or sixteen bits deep, and they need a header read before anybody can say how wide the picture is.

So a “DCM file opener” is a DICOM viewer by the name people search for it under, and there are three kinds. The program on the disc, if it still runs. A desktop viewer you install, which is the right answer for somebody who reads scans every week and a lot of trouble for somebody with one disc. And a web page, of which there are two sorts that look identical: the ones that upload your file to a server and send a picture back, and the ones that read it in the browser. The way to tell them apart is to load the page, disconnect from the internet, and then open the file. Only the second sort still works.
