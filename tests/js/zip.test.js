/**
 * shared/js/zip.js - the stored-only ZIP writer.
 *
...
import { makeZip } from '../../shared/js/zip.js';
...
test('the central directory is where the end record says', async () => {
  const bytes = await built();
...
test('each central record points at its own local header', async () => {
...
test('the file data follows its header, uncompressed', async () => {
  const bytes = await built();
  const view = new DataView(bytes.buffer);
...
test('the sizes are the real ones and are equal', async () => {
  const bytes = await built();
  const view = new DataView(bytes.buffer);
...
test('each entry carries the CRC of its own data', async () => {
  const bytes = await built();
  const view = new DataView(bytes.buffer);
  assert.equal(view.getUint32(14, true), crc32([files[0].data]));
});

test('names are flagged as UTF-8', async () => {
...
test('an empty archive is just an end record', async () => {
  const bytes = await built([]);
...
test('an empty file is allowed', async () => {
...
test('total length is the sum of its parts', async () => {
  const bytes = await built();
