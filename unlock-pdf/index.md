...
This page is an interactive tool that runs entirely in your browser, at https://abox.tools/unlock-pdf/ — nothing you give it is uploaded. What follows is everything the page says about the tool in words; to use it, open the address.
...
[How to unlock a PDF, and which kind of locked it is](https://abox.tools/guides/unlock-a-pdf/): A PDF that will not print and a PDF that will not open are two different problems with one name. What each of them actually is, why one comes off in a click and the other cannot come off at all, and how to tell which you have.

## Also in the box

- [PDF Protector](https://abox.tools/protect-pdf/): Lock a document you would rather not hand to a website in order to lock it.
...
- **The finished file is opened again, here, before you are offered it.** Everything above is this tool marking its own homework until that happens. So the bytes that are about to become your download are handed to the same reader that [the compressor](https://abox.tools/compress-pdf/), [the merger](https://abox.tools/merge-pdf/) and [the redactor](https://abox.tools/redact-pdf/) use — a reader that refuses an encrypted document outright — and it has to open with no password and come back with the same number of pages. A file that still had any encryption on it could not pass that. If it does not, the run is reported as failed and there is no download.
...
The files worth reading first are `config/site.toml` for the Content-Security-Policy, `src/shared/pdf-crypt.js` for how a password becomes a document's key — and for the fact that there is no loop in it anywhere — and `src/shared/aes.js` and `src/shared/rc4.js` for the two ciphers themselves. None of them can reach the network, and neither can the reader or the writer beside them.
